<template>
  <div class="hello">
    ...
    <el-breadcrumb-item>赔偿管理</el-breadcrumb-item>
  </el-breadcrumb>
  <!-- 列表主体部分-->
  <el-card>
    <el-row :gutter="25"><!-- 行内列之间的间隔 -->
      ...
      <el-col :span="3.5">
        <el-date-picker v-model="queryInfo.startDate" type="date" placeholder="赔偿开始日期" :size="size"/>
      </el-col>
      <el-col :span="1.5" class="el-col-h4">
        <h4>赔偿终止日期:</h4>
      </el-col>
      <el-col :span="3.5">
        <el-date-picker v-model="queryInfo.endDate" type="date" placeholder="赔偿终止日期" :size="size"/>
      </el-col>
      <el-col :span="2">
          <el-button type="primary" @click="getCompensateList">检索</el-button>
      </el-col>
      <el-col :span="1">
        <el-button type="primary" @click="addDialogVisible=true;console.log(addDialogVisible)">新的赔偿</el-button>
      </el-col>
    </el-row>
    <!--一览 stripe隔行变色-->
    <el-table :data="compensateList" class="user_list" border stripe>
      <el-table-column prop="id"  type="index" /><!-- 索引列-->
      <el-table-column prop="minsuName" label="赔偿发生的民宿" />
      <el-table-column prop="prodName" label="商品名称"/>
      <el-table-column prop="userName" label="用户名称"/>
      <el-table-column prop="date" label="赔偿日期" />
      <el-table-column prop="price" label="赔偿金额"  />
      <el-table-column prop="reason" label="缘由" />
      <el-table-column label="操作" #default="scope">
        <el-button type="primary" class="mini" @click="showEditCompensateDialog(scope.row.id, true)"><el-icon class="el-input__icon"><Edit/></el-icon></el-button>
        <el-button type="danger" class="mini" @click="deleteCompensate(scope.row.id)"><el-icon class="el-input__icon"><Delete/></el-icon></el-button>
      </el-table-column>
    </el-table>
    <!-- 分页区-->
  ...
  <h2>总计：{{ sum }}元</h2>
    ...
    <el-dialog title="添加"  v-model="addDialogVisible" width="50%" align-center @closs="addDialogClosed">
      <el-form :model="addForm" :rules="formRules" ref="addFormRef" label-width="120px">
        ...
        <el-form-item label="赔偿人员" prop="userId" required>
        ...
        <el-form-item label="赔偿日期" prop="date" required>
          <el-date-picker v-model="addForm.date" type="date" placeholder="赔偿日期" :size="size"/>
        </el-form-item>
        <el-form-item label="赔偿金额" prop="price">
          <el-input v-model="addForm.price"></el-input>
        </el-form-item>
        <el-form-item label="赔偿缘由" prop="reason" >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addCompensate">确 定</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="修改"  v-model="editDialogVisible" width="50%" align-center @closs="addDialogClosed">
      <el-form :model="updateForm" :rules="formRules" ref="updateFormRef" label-width="100px">
        ...
        <el-form-item label="赔偿日期" prop="date" required>
          <el-date-picker v-model="updateForm.date" type="date" placeholder="赔偿日期" :size="size"/>
        </el-form-item>
        <el-form-item label="赔偿金额" prop="price">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="updateCompensate">确 定</el-button>
      </span>
    </el-dialog>
  </el-card>
  </div>
...
export default {
  ...
  methods:{
    // 一览数据取得
    async getCompensateList(){
      this.compensateList=[];
      this.sum = 0;
      const {data:res} = await this.$http.get("allCompensate",{ params:this.queryInfo});
      
      if(res.CompensateCounts > 0){
        this.compensateList = res.data;
        this.compensateList.forEach (item => {
          this.sum += item.price;
        });
        this.total = res.CompensateCounts;
      }
    },
    // 每页最大数变化处理
    ...
    async addCompensate(){
      this.$refs.addFormRef.validate(async valid=>{
        console.log(valid);
        if(!valid)return;
        const {data:res} = await this.$http.post("addCompensate",this.addForm); 
        if(res != "success"){
    ...
    async deleteCompensate(id){
      ...
      if(confirmResult == "confirm"){ // 确定
        const {data:res} = await this.$http.delete("deleteCompensate?id="+id);
        if(res != "success"){
    ...
    // 获取要编辑的信息
    async showEditCompensateDialog(id, editFlg){
      const {data:res} = await this.$http.get("getCompensate?id="+id);
    ...
    async updateCompensate(){
      // 表单验证
       this.$refs.updateFormRef.validate(async valid =>{
        ...
        // 进行更新
        const {data:res} = await this.$http.put("updateCompensate", this.updateForm);
         if(res != "success"){
